...
/* User's own messages: a rounded bubble that hugs its content and sits
   ...
   normal light text colour, rather than a loud accent fill. The bubble
   is also made a flex container with centered content, so the text
   sits in the true middle of the rectangle on both axes -- horizontally
   AND vertically -- rather than relying on text-align plus padding
   alone (which only centers horizontally and can look off-center
   vertically once the bubble wraps to two lines). */
